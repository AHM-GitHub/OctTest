...
//@Test
public class browserTestNG2 {          // E:\Eclipse-Data\workspaces\Eclipse-inst-win64-Data-Neon\Selenium_313\src\practices
...
//		JavascriptExecutor js = (JavascriptExecutor) driver;
	...
	@Test(priority = 0, enabled = false ) //true)
	...
	@Test(priority = 1, enabled = false)  //  @Test(priority = 1, enabled = true) 
	...
	public void Test_three_Table_Sort_And_Search_Demo() {
		...
		driver.findElement(By.xpath("//a[@class='list-group-item'][contains(text(),'Table Sort & Search')]")).click(); waiting(5000);
		
		
		driver.findElement(By.xpath("//a[@id='example_next']")).click(); waiting(5000);  // Click next btn on lower left
		...
		js.executeScript("window.scrollBy(0,700)");   waiting(5000); // scroll down to near bottom thru java scripts
		//  https://www.guru99.com/scroll-up-down-selenium-webdriver.html
		js.executeScript("window.scrollBy(0,-700)");   waiting(5000); // scroll up  thru java scripts
		
		
		// Below to click search box  to be complete
		driver.findElement(By.xpath("//input[@type='search']")).click(); waiting(5000);  // click search box
	...
	@Test(priority = 3, enabled =  false)     //true)      // false)
	...
	@Test(priority = 4, enabled = false)  //false) // true)  
	...
	@Ignore
	@Test(priority = 0, expectedExceptions = ElementNotInteractableException.class) 
